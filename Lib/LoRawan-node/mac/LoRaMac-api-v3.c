...
uint8_t LoRaMacSendFrame( uint8_t fPort, void *fBuffer, uint16_t fBufferSize )
{
    ...
    DEBUG(2,"%s\r\n",__func__);
    DEBUG(2,"LoRaMacState &= ~MAC_TX_RUNNING--6 %d\r\n",LoRaMacState);
...
uint8_t LoRaMacSendConfirmedFrame( uint8_t fPort, void *fBuffer, uint16_t fBufferSize, uint8_t nbRetries )
{
	...
	DEBUG(3,"mibGet = %d, Datarate = %d\r\n",MIB_CHANNELS_DATARATE, mibGet.Param.ChannelsDatarate);
...
void LoRaMacGMulticast( void )
{
    MibRequestConfirm_t mibGet;

    mibGet.Type = MIB_MULTICAST_CHANNEL;
    LoRaMacMibGetRequestConfirm( &mibGet );
}
